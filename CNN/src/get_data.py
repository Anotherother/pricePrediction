...
#"AND pair.id = candlestick.pair_id AND exchange.name = '" + exchange + 
#"' AND candlestick.pair_id IN (SELECT pair.id FROM pair "
#"WHERE pair.alias_id = (SELECT pair.id FROM pair WHERE pair.name = '" + pair + "') OR pair.name = '" + pair + "');"
...
def get_data_frame5minutes(pair: str = 'USDT_BTC', exchange: str = 'poloniex') -> pd.DataFrame:
    ...
    df = pd.read_sql_query(
        "SELECT open,close,low,high,volume,date+time as date,adj_close,exchange.name AS exchange,pair.name AS pair "
            "FROM candlestick, exchange, pair "
            "WHERE candlestick.exchange_id = exchange.id "
            "AND candlestick.pair_id = pair.id AND exchange.name = '" + exchange + "' AND pair.name = '" + pair + 
            "' ORDER BY candlestick.date, candlestick.time"
